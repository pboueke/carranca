...
if ! command -v iptables >/dev/null 2>&1; then
  _log "WARNING: iptables not available — network policy not enforced"
  exec /usr/local/bin/shell-wrapper.sh
fi

# Default policy: drop all outbound traffic
iptables -P OUTPUT DROP 2>/dev/null || {
  _log "WARNING: iptables failed (likely insufficient privileges) — network policy not enforced"
  exec /usr/local/bin/shell-wrapper.sh
}
